...
const removeSmallest = numbers => {
  ...
  let ratingsRemaining = []
  
  // variable to store lowest rating
  let lowestRating = Math.min(...numbers)
  
  // for each number in the list, add to ratingsRemaining if it is not equal to lowestRating
  numbers.forEach(num => {
    if(num !== lowestRating) ratingsRemaining.push(num)
  })
  
  // create an array of all the lowerst nums
  let arrayOfLowest = numbers.filter((num, i) => num === lowestRating);

  // if there is more than one item in the array, add one of them to the ratingsRemaining
  if (arrayOfLowest.length > 1) {
    ratingsRemaining.push(arrayOfLowest[0])
  }
  
  // return new array
  return ratingsRemaining
}

let ratings = [2, 2, 1, 2, 1]
removeSmallest(ratings)
