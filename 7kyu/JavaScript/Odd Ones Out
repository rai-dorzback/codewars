...
let oddOnesOut = n => {
  ...
  // create a count of how many times each number appears
  for (let num of n) {
    // if count object already has a property for that number, add to its count
    if (num in count) {
      count[num] += 1
    // else, create a key for that number and start counting at 1
    } else {
      count[num] = 1
    }
  }
  // filter the numbers list for items whos counts were even
  return n.filter((num) => count[num] % 2 === 0)
}

// METHOD 2: more concise version of METHOD 1
let oddOnesOut = n => {
  count = {}
  
  for (let num of n) {
    num in count ? count[num] += 1 : count[num] = 1
  }
