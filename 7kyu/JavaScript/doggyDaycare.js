...
// method 2
Dog.prototype.checkDog = function() {
  // make possible responses an array
  const responses = [`${this.name} can be accepted`, `${this.name} can only be accepted by itself`, `${this.name} can not be accepted`]
  // ternary operator
  return (this.vaccinated && this.wormed) ? responses[0] : (this.vaccinated || this.wormed) ? responses[1] : responses[2];
  };
