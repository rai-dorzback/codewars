...
class Ship:
    ...
    def is_worth_it(self):
        draftAfterCrew = self.draft - self.crew * 1.5
        if draftAfterCrew > 20:
            return True
        return False

# METHOD 2: lambda & put if/else on one line
class Ship:
    def __init__(self, draft, crew):
        self.draft = draft
        self.crew = crew
        
    is_worth_it = lambda self : True if self.draft - self.crew * 1.5 > 20 else False
